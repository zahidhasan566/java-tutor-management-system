...
public class Available_Teacher1 extends JFrame implements ActionListener
{
	...
	String uid;
	public Available_Teacher1(String id)
	{
		super (" Show_Teacher ");
		...
		//String [][]row;
		String []col ={"ID", "Name", "Phone", "Area", "Subject", "Wage", "Mail", "Ratings"};
		...
		try
		{
			String query1="SELECT teacher.*, AVG(rating.RateValue) AS RateValue, preference.IsMail AS Mail1, preference.IsPhone As Phone1 FROM teacher,rating,preference WHERE TID_ra=TID_ti AND TID_ti= Tid_tp GROUP BY TID_ti;";
			...
			while(rs.next()){
				...
				model.addRow(new Object[]{Integer.toString(rs.getInt("TID_ti")),
										 ...
										 prefm,
										 Double.toString(rs.getDouble("RateValue"))});
			}
	...
	{  
		...
		{
			Available_Teacher l  =new Available_Teacher(uid);
			l.setVisible(true);
			this.setVisible(false);
		}
		else{}
