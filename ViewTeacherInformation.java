...
import java.util.*;
import java.sql.*; 
public class ViewTeacherInformation extends JFrame implements ActionListener 
{
	...
	public ViewTeacherInformation(String id)
	{
		...
		try
		{
			...
			nameTF.setText(rs.getString("TeacherName"));
			phnTF.setText(Integer.toString(rs.getInt("PhoneNumber")));
			areaTF.setText(rs.getString("Area"));							 
			int sub=(rs.getInt("Subjectcode_aa"));
				...
				{
					subjTF1.setText("Bangla");
				}
				else if(sub==103)
				{
					subjTF1.setText("Maths");
				}
			
			wageTF.setText(Integer.toString(rs.getInt("Wage")));
			mailTF.setText(rs.getString("Mail"));
			idTF.setText(id);			
			}
