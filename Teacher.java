...
public class Teacher extends JFrame implements ActionListener
{
	...
	public Teacher(String id)
	{
		...
		userLabel=new JLabel("WELCOME TEACHER!!!! ");
		userLabel.setBounds(300,20,700,100);
		userLabel.setForeground(Color.WHITE);
		panel.add(userLabel);
		 
		viewstudent=new JButton("VIEW MY STUDENTS");
		...
		searchstudent=new JButton("AVAILABLE STUDENTS");
	...
	{  
		...
		{   
			ViewTeacherInformation v  =new ViewTeacherInformation(uid);	
			v.setVisible(true);
			this.setVisible(false);
		}
		else if (ae.getSource()==searchstudent)
		{   
			Available_Student v  =new Available_Student(uid);	
			v.setVisible(true);
			this.setVisible(false);
		}
		else if (ae.getSource()==viewstudent)
		{   
			ViewMyStudent v  =new ViewMyStudent(uid);	
			v.setVisible(true);
			this.setVisible(false);
		}
		else if (ae.getSource()==updateinformation)
		{   
			Update_Teacher v  =new Update_Teacher(uid);	
			v.setVisible(true);
			this.setVisible(false);
		}
	}
